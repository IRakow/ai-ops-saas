...
class AIOpsService:
    ...
    def authenticate_user(self, email, password):
        ...
        try:
            result = execute_with_retry(
                lambda: self.supabase.table("ai_ops_users")
                .select("*")
    ...
    def create_user(self, name, email, password, phone=None):
        ...
        result = self.supabase.table("ai_ops_users").insert({
            "name": name,
    ...
    def create_session(self, user_id, mode, title=None):
        """Create a new AI Ops session."""
        result = self.supabase.table("ai_ops_sessions").insert({
            "user_id": user_id,
            "mode": mode,
            "title": title,
    ...
    def get_session(self, session_id):
        """Get a session by ID."""
        result = execute_with_retry(
            lambda: self.supabase.table("ai_ops_sessions")
    ...
    def list_attention_sessions(self):
    ...
    def add_message(self, session_id, sender_type, sender_name, content,
        ...
        result = self.supabase.table("ai_ops_messages").insert({
    ...
    def create_tasks(self, session_id, tasks):
    ...
    def add_file(self, session_id, filename, gcs_path=None, gcs_url=None):
        ...
        result = self.supabase.table("ai_ops_files").insert({
            "session_id": session_id,
            "filename": filename,
            "gcs_path": gcs_path,
            "gcs_url": gcs_url,
    ...
    def log_audit(self, session_id, user_id, event_type, details=None):
        ...
        try:
            self.supabase.table("ai_ops_audit_log").insert({
                "session_id": session_id,
                "user_id": user_id,
    ...
    def get_audit_log(self, session_id=None, limit=50):
    ...
    def queue_task(self, session_id, task_type, description, attachments=None,
                   phase="execute", understanding_output=None):
    ...
    def get_pending_queue_item(self):
        ...
        """
        result = execute_with_retry(
            lambda: self.supabase.table("ai_ops_agent_queue")
            .select("*")
            .eq("status", "pending")
